...
const ARTIFACTS_FOLDER = path.resolve('src', 'database', 'temp', 'artifacts')
...
const main = async () => {
  ...
  for (let i = 0; i < PARTS_COUNT; i++) {
    ...
    const artifactDatabasePath = path.resolve(
      ARTIFACTS_FOLDER,
      `temp-artifact-processed-${i}`,
      `${i}.json`
    )
    const partArtifactDatabase = JSON.parse(
      await readFileAsync(artifactDatabasePath)
    )
